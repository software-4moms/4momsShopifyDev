...
const selectors = {
  ...
  ajaxCartQuantityText: '[data-ajax-cart-quantity-text]',
  ajaxCartAffirm: '[data-ajax-cart-affirm]'
};
...
export default class AJAXCart {
  ...
  constructor(templateData = {}) {
    ...
    this.$ajaxCartQuantityText = $(selectors.ajaxCartQuantityText);
    this.$ajaxCartAffirm       = $(selectors.ajaxCartAffirm);
  ...
  updateCartCount(cart) {
    ...
    // inject the correct singular or plural text on update
    if (cart.item_count > 1) {
      this.$ajaxCartQuantityText.html(this.$ajaxCartQuantityText.data().ajaxCartQuantityTextPlural);
    } else {
      this.$ajaxCartQuantityText.html(this.$ajaxCartQuantityText.data().ajaxCartQuantityTextSingle);
    }

    // we want to refresh affirm each time that the cart updates
    if (cart.item_count > 0) {
      this.$ajaxCartAffirm.attr('data-amount', cart.total_price);
      window.affirm.ui.refresh();
    }

    //  update the header nav cart count
    if (cart.item_count) {
  ...
  onRender(e) {
    if (e.cart) {
      ...
      if (e.cart.item_count === 0) {
        ...
        objectFitPolyfill();
        // if (typeof window.LimeSpot["Shopify"] === "object" ){
        //   window.LimeSpot.Shopify.AjaxCart.Render({ containerQuerySelector: '#shopify-section-ajax-cart', cartInnerQuerySelector: '.ajax-cart__footer-row--limespot' });
        // }
          // window.LimeSpot.Shopify.AjaxCart.Render({ containerQuerySelector: '#shopify-section-ajax-cart', cartInnerQuerySelector: '.ajax-cart__footer-row--limespot' });
    ...
    // extend code
    window.dispatchEvent(window.Extend.CustomEvent("refreshCart"));
